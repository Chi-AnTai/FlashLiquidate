...
async function deployOracle(erc20Address, erc721Address) {
    ...
    await oracle.setDirectPrice(erc721Address, oraclePrice.mul(30))
    return oracle
}
...
async function deployCErc721(underlyingAddress) {
    const cErc721Contract = await hre.ethers.getContractFactory("CERC721");
    const cErc721 = await cErc721Contract.deploy(underlyingAddress)
    console.log('cErc721 address', cErc721.address)
    return cErc721
}
...
describe("Deploy Test", function () {
    ...
    this.beforeAll("Deploy", async () => {
        ...
        erc20 = erc
        cerc20 = cerc
        erc721 = await deployErc721(deployer)
        cErc721 = await deployCErc721(erc721.address)
        oracle = await deployOracle(erc20.address, erc721.address)
    ...
    this.beforeAll("Set parameter", async () => {
        ...
        await comptroller._supportMarket(cErc721.address)
        await comptroller.enterMarkets([cErc721.address])
